...
function formatWithHeaderFooter(message) {
    try {
        ...
        const header = getSetting('company_header', 'ArknetDS BOT');
...
async function connectToWhatsApp() {
    try {
        ...
        sock = makeWASocket({
            auth: state,
            logger,
            browser: ['Arknet Digital Solution', 'Chrome', '1.0.0'],
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    ...
                    `💰 *Dukungan Pengembang:*\n` +
                    `• DANA: 085352525599\n` +
                    `• BRI: 064901001164532 a.n Yudi Kendengis\n\n` +
...
▸ *setfooter [teks_footer_baru]* — Ganti footer pesan bot
   Contoh: setfooter Powered by Arknet Digital Solution
...
async function handleChangeSSID(senderNumber, remoteJid, params) {
    try {
        ...
        if (!device) {
            await sock.sendMessage(remoteJid, { 
                text: `${getSetting('company_header', 'ArknetDS BOT')}
...
Mohon tunggu sebentar.${getSetting('footer_info', 'Internet Tanpa Batas')}`
        ...
        let responseMessage = `${getSetting('company_header', 'ArknetDS BOT')}
...
Mohon tunggu sebentar.${getSetting('footer_info', 'Internet Tanpa Batas')}`
        ...
        const adminResponseMessage = `${getSetting('company_header', 'ArknetDS BOT')}
...
_Perubahan selesai pada: ${new Date().toLocaleString()}_${getSetting('footer_info', 'Internet Tanpa Batas')}`;
        ...
        try {
            ...
            const customerNotificationMessage = `${getSetting('company_header', 'ArknetDS BOT')}
...
Mohon tunggu sebentar.${getSetting('footer_info', 'Internet Tanpa Batas')}`
        ...
        // Pesan sukses untuk admin
        let adminResponseMessage = `${getSetting('company_header', 'ArknetDS BOT')}
...
async function sendWelcomeMessage(remoteJid, isAdmin = false) {
    try {
        ...
        let welcomeMessage = `👋 *Selamat Datang di Bot WhatsApp ${getSetting('company_header', 'ArknetDS BOT') || 'ISP Monitor'}*\n\n`;
        ...
        // Tambahkan footer
        welcomeMessage += `🏢 *${getSetting('company_header', 'ArknetDS BOT') || 'ISP Monitor'}*\n`;
...
Pesan GenieACS telah diaktifkan kembali.`);
        ...
        // Jika GenieACS dinonaktifkan, abaikan semua perintah kecuali dari nomor 6281259872053
        if (!genieacsCommandsEnabled && senderNumber !== '6281259872053') {
            // Hanya nomor 6281259872053 yang bisa menggunakan bot saat GenieACS dinonaktifkan
...
async function handleAdminMenu(remoteJid) {
    try {
        ...
        // Tambahkan footer
        adminMessage += `🏢 *${getSetting('company_header', 'ArknetDS BOT') || 'ISP Monitor'}*\n`;
